...
def _find_link_without_keyword(data, contacts):
    found_link = find_contact(data, FIND.LINKEDIN_LINK, '', WORD_POSITION.NONE)
    while found_link:
...
def _result_of_search(data, contacts, keyword):
    result = __find_linkedin_contacts(data, keyword)
    data = __delete_found_info(data, result, keyword)
    contacts += result
    return data, contacts


class LinkedinFinder(AbstractFinder):
    ...
    def find(self, data='', correct_format=CONTACT_FORMAT.CORRECT, search_contacts=SEARCH_CONTACTS.STRICT):
        try:
            ...
            if search_contacts:
                found_keyword = find_contact(data, FIND.KEYWORD_LINKEDIN, '', WORD_POSITION.NONE)
                while found_keyword:
