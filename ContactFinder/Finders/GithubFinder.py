...
def __find_contact_after_keyword(data, keyword):
    found_contact = find_contact(data, FIND.GITHUB_USERNAME, keyword, WORD_POSITION.AFTER)
    if found_contact and not check_contact(data, found_contact, keyword, WORD_POSITION.AFTER, KIND_CONTACT.USERNAME):
        found_contact = ''
    return found_contact


def __find_github_contacts(data, keyword):
    result = []
    while True:
...
def _find_link_without_keyword(data, contacts):
    found_link = find_contact(data, FIND.GITHUB_LINK, '', WORD_POSITION.NONE)
    while found_link:
...
def _result_of_search(data, contacts, keyword):
    result = []
    result += __find_github_contacts(data, keyword)
    data = __delete_found_info(data, result, keyword)
    contacts += result
    return data, contacts


class GithubFinder(AbstractFinder):
    ...
    def find(self, data='', correct_format=CONTACT_FORMAT.CORRECT, search_contacts=SEARCH_CONTACTS.STRICT):
        try:
            ...
            if search_contacts:
                found_keyword = find_contact(data, FIND.KEYWORD_GITHUB, '', WORD_POSITION.NONE)
                while found_keyword:
