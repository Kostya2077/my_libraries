...
class DribbbleFinder(AbstractFinder):
    ...
    def find(self, data='', correct_format=CONTACT_FORMAT.CORRECT, search_contacts=SEARCH_CONTACTS.STRICT):
        try:
            result = []
            while find_contact(data, FIND.DRIBBBLE_LINK, '', WORD_POSITION.NONE):
                found_contact = find_contact(data, FIND.DRIBBBLE_LINK, '', WORD_POSITION.NONE)
