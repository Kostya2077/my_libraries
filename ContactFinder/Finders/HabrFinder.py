...
class HabrFinder(AbstractFinder):
    ...
    def find(self, data='', correct_format=CONTACT_FORMAT.CORRECT, search_contacts=SEARCH_CONTACTS.STRICT):
        try:
            result = []
            while find_contact(data, FIND.HABR_LINK, '', WORD_POSITION.NONE):
                found_contact = find_contact(data, FIND.HABR_LINK, '', WORD_POSITION.NONE)
                if check_contact(data, found_contact, '', WORD_POSITION.NONE, KIND_CONTACT.LINK) and \
                        not re.search('career[.]habr', found_contact):
